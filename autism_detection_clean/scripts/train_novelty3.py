"""
Train with ALL 3 NOVELTIES (146 features)
Baseline: 136 features → 90.08%
+Novelty 1: +1 Symmetry
+Novelty 2: +4 Entropy
+Novelty 3: +5 Jerk → 146 total
"""
...
def train_all_novelties():
    print("\n" + "=" * 80)
    print("TRAINING WITH ALL 3 NOVELTIES")
    print("=" * 80)
    print("Baseline: 136 features → 90.08% accuracy")
    print("Novelty 1: +1 Bilateral Symmetry")
    print("Novelty 2: +4 Motion Entropy")
    print("Novelty 3: +5 Jerk Analysis")
    print("TOTAL: 146 features")
    print("=" * 80)
    
    if not os.path.exists('X_novelty3.npy'):
        print("ERROR: X_novelty3.npy not found!")
        print("Run: python scripts/add_novelty3_jerk.py")
        return
    
    X = np.load('X_novelty3.npy')
    Y = np.load('Y_novelty3.npy')
    ...
    results = {
        'novelties': 'Novelty 1 (Symmetry) + Novelty 2 (Motion Entropy) + Novelty 3 (Jerk Analysis)',
        'features': 146,
    ...
    with open('kfold_results_novelty3.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"✓ Results saved: kfold_results_novelty3.json\n")
