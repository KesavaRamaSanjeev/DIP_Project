"""
Train with NOVELTY 1 ONLY (137 features)
Baseline: 136 features → 90.08% accuracy
Novelty 1: +1 Bilateral Symmetry feature
"""
...
def train_novelty1():
    print("\n" + "=" * 80)
    print("TRAINING WITH NOVELTY 1: BILATERAL SYMMETRY")
    print("=" * 80)
    print("Baseline: 136 features → 90.08% accuracy")
    print("Novelty 1: +1 symmetry feature → 137 total")
    print("=" * 80)
    
    # Load features
    if not os.path.exists('X_novelty1.npy'):
        print("ERROR: X_novelty1.npy not found!")
        print("Run: python scripts/add_novelty1_symmetry.py")
        return
    
    X = np.load('X_novelty1.npy')
    Y = np.load('Y_novelty1.npy')
    ...
    results = {
        'novelty': 'Novelty 1 - Bilateral Symmetry',
        'features': 137,
    ...
    with open('kfold_results_novelty1.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"✓ Results saved: kfold_results_novelty1.json\n")
