"""
K-Fold Validation with Behavioral Features
============================================
Stratified 5-fold cross-validation using:
- LSTM features (128 dims - temporal patterns)
- Motion features (8 dims - optical flow)
- Behavioral features (4 dims - NEW: repetition, smoothness, spatial consistency, acceleration)
- Total: 153 dims

With class weight balancing for 74/26 autism/normal split
"""
...
def train_kfold_behavior():
    """
    Stratified 5-fold validation with behavioral features
    """
    
    print(f"\n{'='*70}")
    print(f"K-FOLD VALIDATION: With Behavioral Features (153 dims)")
    print(f"{'='*70}\n")
    
    # Load features
    print("Loading features...")
    X_combined = np.load('X_combined_behavior.npy')
    Y_labels = np.load('Y_labels_behavior.npy')
    
    print(f"  Shape: {X_combined.shape}")
    print(f"  Labels: {np.sum(Y_labels == 1)} autism, {np.sum(Y_labels == 0)} normal")
    print(f"  Features: LSTM (128) + Motion (8) + Placeholder (13) + Behavior (4) = 153\n")
    ...
    print(f"{'='*70}")
    print(f"Running 5-Fold Cross-Validation (with class weighting)")
    print(f"{'='*70}\n")
    ...
    for train_idx, test_idx in skf.split(X_combined, Y_labels):
        ...
        # ====== SVM with GridSearch + Class Weighting ======
    ...
    print(f"\n{'='*70}")
    print(f"FINAL RESULT: {mean_acc*100:.2f}% +/- {std_acc*100:.2f}%")
    print(f"{'='*70}\n")
    
    # Save results
    np.save('kfold_results_behavior.npy', {
    ...
    print("Results saved to kfold_results_behavior.npy\n")


if __name__ == "__main__":
    train_kfold_behavior()
