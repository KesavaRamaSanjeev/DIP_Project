...
def extract_features_from_raw():
    ...
    print(f"🎬 PHASE 1: Feature Extraction from Raw Videos")
    ...
    for i, video_path in enumerate(all_files):
        ...
        try:
            ...
            if file_size < 10000:  # Less than 10KB is likely corrupted
                print(f"❌ Corrupted (size: {file_size} bytes)")
                failed_count += 1
                continue
            ...
            if video_tensor.shape != (2, 16, 112, 112):
                print(f"❌ Invalid shape {video_tensor.shape}")
                failed_count += 1
                continue
            ...
            try:
                ...
                print(f"⚠️ Autism feature extraction failed ({str(autism_err)[:30]}), using zeros")
...
def detect_and_remove_duplicates(X_combined, Y_labels, valid_files):
    ...
    print(f"\n🗑️  Deleting duplicate video files from dataset_new/...\n")
    for idx in duplicates_to_remove:
        ...
        try:
            ...
            print(f"  ✅ Removed: {os.path.basename(video_path)}")
            if os.path.exists(motion_path):
                os.remove(motion_path)
        except Exception as e:
            print(f"  ⚠️  Could not remove {os.path.basename(video_path)}: {str(e)}")
